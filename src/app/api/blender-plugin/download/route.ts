...
/**
 * GET /api/blender-plugin/download
 * 
 * Creates a zip file of the blender-plugin directory and streams it to the client
 */
export async function GET(_request: NextRequest) {
  try {
    const pluginDir = join(process.cwd(), 'blender-plugin', 'concepto_blender_plugin');
    ...
    headers.set('Content-Disposition', 'attachment; filename="concepto_blender_plugin.zip"');
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        addDirectoryToArchive(archive, pluginDir, 'concepto_blender_plugin');
