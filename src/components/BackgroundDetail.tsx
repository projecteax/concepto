...
export function BackgroundDetail({
  ...
  const onSave = React.useCallback((b: GlobalAsset) => { if (isReadOnly) return; onSaveProp(b); }, [isReadOnly, onSaveProp]);
    ...
    >
      {isReadOnly && (
        ...
        <div className="px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-800 text-sm">
          Viewer mode: background details are read-only.
        </div>
        </>
      ...
      <AppBreadcrumbHeader
        ...
        backHref={`${basePath}/shows/${show.id}/assets?category=background`}
        items={[
          { label: show.name, href: `${basePath}/shows/${show.id}` },
          { label: 'Assets', href: `${basePath}/shows/${show.id}/assets` },
          { label: 'Backgrounds', href: `${basePath}/shows/${show.id}/assets?category=background` },
          { label: background.name || 'Background' },
        ]}
        subtitle="Background asset"
        actions={
