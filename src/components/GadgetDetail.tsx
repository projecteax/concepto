...
export function GadgetDetail({
  ...
  const onSave = React.useCallback((updatedGadget: GlobalAsset) => {
    if (isReadOnly) return;
    onSaveProp(updatedGadget);
    ...
    >
      {isReadOnly && (
        ...
        <div className="px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-800 text-sm">
          Viewer mode: gadget details are read-only.
        </div>
        </>
      ...
      <AppBreadcrumbHeader
        ...
        backHref={`${basePath}/shows/${show.id}/assets?category=gadget`}
        items={[
          { label: show.name, href: `${basePath}/shows/${show.id}` },
          { label: 'Assets', href: `${basePath}/shows/${show.id}/assets` },
          { label: 'Gadgets', href: `${basePath}/shows/${show.id}/assets?category=gadget` },
          { label: gadget.name || 'Gadget' },
        ]}
        subtitle="Gadget asset"
        actions={
