...
export function CharacterDetail({
  ...
  const onSave = React.useCallback((updatedCharacter: Character) => {
    if (isReadOnly) return;
    onSaveProp(updatedCharacter);
  ...
  const onAddConcept = React.useCallback((concept: Omit<AssetConcept, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (isReadOnly) return;
    onAddConceptProp(concept);
  }, [isReadOnly, onAddConceptProp]);
    ...
    >
      {isReadOnly && (
        ...
        <div className="px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-800 text-sm">
          Viewer mode: character details are read-only.
        </div>
        </>
